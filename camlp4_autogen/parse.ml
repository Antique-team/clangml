...
type sum_type_branch = string * ocaml_type list
  deriving (Show)
...
let rec ast_type_to_type (ctyp: Ast.ctyp) : ocaml_type =
  match ctyp with
...
let ast_str_item_to_sum_type (str_item : Ast.str_item) : sum_type option =
  match str_item with
...
let parse_file (file_path : string) : sum_type list =
