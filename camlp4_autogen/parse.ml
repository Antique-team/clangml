...
and sum_type_branch = string * (ocaml_type list)


and ocaml_type =
	| NamedType of string
	| ListOfType of ocaml_type
	(* Will want others, eventually *)
	

let rec parse_file (file_path : string) : sum_type list =
  ...
  let str_items = Ast.list_of_str_item str_item [] in
	...
	*)
	let accumulate_sum_type (str_item  : Ast.str_item) (sum_types : sum_type list) : sum_type list =
		match ast_str_item_to_sum_type str_item with
			| Some sum_type -> sum_type::sum_types
			| None -> sum_types
	in
...
and flatten_ast_sum_type_branches (t : Ast.ctyp) : Ast.ctyp list =
	match t with 
		| Ast.TyOr _ -> 
			begin 
				let rec flatten_sum_type_contents_aux t flattened_contents =
					...
					| Ast.TyOr (_, left, right) ->
						let with_right_contents = flatten_sum_type_contents_aux right flattened_contents in
...
and flatten_ast_tuple_type_components (t : Ast.ctyp) : Ast.ctyp list =
	match t with 
		| Ast.TyAnd _ -> 
			begin 
				let rec flatten_tuple_type_contents_aux t flattened_contents =
					...
					| Ast.TyAnd (_, left, right) ->
						let with_right_contents = flatten_tuple_type_contents_aux right flattened_contents in
...
and ast_str_item_to_sum_type (str_item : Ast.str_item) : sum_type option =
	match str_item with
		| Ast.StTyp (_, Ast.TyDcl(_,name,_,Ast.TySum(_, ast_branches),_)) -> 
			...
			Some (name, sum_branches)			
		| _ -> None

and ast_sum_type_branch_to_branch (ctyp: Ast.ctyp) : sum_type_branch =
	
	let (identifier, ast_branch_components) = match ctyp with
	| Ast.TyId (_, Ast.IdUid (_, identifier)) -> (identifier, [])
...
and ast_type_to_type (ctyp: Ast.ctyp) : ocaml_type =
	match ctyp with
	| Ast.TyId (_, Ast.IdLid (_, identifier)) -> NamedType(identifier)
