...
let codegen ocaml_types =
            ...
            |> List.fold_left (fun expr (i, ty) ->
                ...
                let mkmap name = function
                  | None ->
                      ...
                      <:expr<v.$lid:"map_" ^ name$ v state $lid:var$>>
                  | Some fn ->
                      let var = mangle name in
                      <:expr<$lid:"map_" ^ fn$ v.$lid:"map_" ^ name$ v state $lid:var$>>
                in
  ...
  let default =
    let mkbinding (name, (_loc, _, _), _) =
      <:rec_binding<$lid:"map_" ^ name$ = $lid:"visit_" ^ name$>>
    in
